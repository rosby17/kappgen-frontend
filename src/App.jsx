                      ...
                      <div className="order-2">
                        ...
                        <div className={`mt-3 space-y-4 ${newChannel.publish_mode === 'auto' || newChannel.publish_mode === 'scheduled' ? '' : 'hidden'}`}>
                          <div className="grid grid-cols-1 gap-2 md:grid-cols-4">
                            ...
                            <label><span className="mb-1.5 block text-[10px] font-bold text-slate-400">Programmer</span><span className="flex h-10 items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--bg-surface-alt)] px-3"><span className="material-symbols-outlined text-[15px] text-[#00c2ff]">calendar_month</span><input type="date" value={newChannel.publish_schedule_date || ''} onChange={e => setNewChannel({ ...newChannel, publish_schedule_date: e.target.value, publish_mode: 'scheduled' })} style={{ colorScheme: 'dark' }} className="min-w-0 flex-1 bg-transparent text-[11px] text-white outline-none [color-scheme:dark]" /></span></label>
