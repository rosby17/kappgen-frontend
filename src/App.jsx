                  ...
                  return (
                    <div className="flex flex-col space-y-6">
                      ...
                      <div className="order-2">
                        ...
                        <div className={`mt-3 flex flex-wrap items-start gap-2 ${newChannel.publish_mode === 'auto' || newChannel.publish_mode === 'scheduled' ? '' : 'hidden'}`}>
                          <div className="w-full md:w-[32%]">
                            <span className="block text-[10px] font-bold text-slate-400 mb-1.5">Visibilité par défaut</span>
                            <div className="relative"><button type="button" onClick={() => setVisibilityMenuOpen(v => !v)} className="flex w-full items-center justify-between rounded-xl border border-[var(--border)] bg-[var(--bg-surface-alt)] px-3 py-2 text-left text-[11px] text-white"><span>{[['private','Privée'],['unlisted','Non répertoriée'],['members','Réservée aux membres'],['public','Publique']].find(([id]) => id === (newChannel.youtube_privacy_status || 'public'))?.[1] || 'Publique'}</span><span className="material-symbols-outlined text-[16px]">{visibilityMenuOpen ? 'expand_less' : 'expand_more'}</span></button>{visibilityMenuOpen && <div role="radiogroup" aria-label="Visibilité par défaut" className="absolute left-0 right-0 top-full z-30 mt-1 rounded-xl border border-[var(--border)] bg-[var(--bg-dropdown)] p-1 shadow-2xl">{[['private','Privée','lock'],['unlisted','Non répertoriée','link_off'],['members','Réservée aux membres','group'],['public','Publique','public']].map(([id,label,icon]) => { const selected = (newChannel.youtube_privacy_status || 'public') === id; return <button key={id} type="button" role="radio" aria-checked={selected} onClick={() => { setNewChannel({ ...newChannel, youtube_privacy_status: id, publish_mode: 'manual' }); setVisibilityMenuOpen(false); }} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-[11px] transition-colors ${selected ? 'bg-[#00c2ff]/10 text-white' : 'text-slate-300 hover:bg-white/5'}`}><span className={`flex h-4 w-4 items-center justify-center rounded-full border-2 ${selected ? 'border-[#00c2ff]' : 'border-slate-500'}`}>{selected && <span className="h-2 w-2 rounded-full bg-[#00c2ff]" />}</span><span className="material-symbols-outlined text-[14px] text-slate-400">{icon}</span>{label}</button>; })}</div>}</div>
