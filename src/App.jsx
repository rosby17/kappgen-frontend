...
function DirectHourInput({ value, onChange }) {
  const format = hour => `${String(hour ?? 0).padStart(2, '0')}:00`;
  const [draft, setDraft] = useState(format(value));
  useEffect(() => setDraft(format(value)), [value]);
  const commit = () => {
    const hour = Number.parseInt(draft.replace('h', ':').split(':')[0], 10);
    if (Number.isFinite(hour) && hour >= 0 && hour <= 23) {
      onChange(hour);
      setDraft(format(hour));
    } else {
      setDraft(format(value));
    }
  };
  return <input type="text" inputMode="numeric" value={draft} onFocus={e => e.target.select()} onChange={e => setDraft(e.target.value.slice(0, 5))} onBlur={commit} onKeyDown={e => { if (e.key === 'Enter') e.currentTarget.blur(); }} className="h-10 w-full rounded-xl border border-[var(--border)] bg-[var(--bg-surface-alt)] px-3 text-[11px] font-bold text-white outline-none transition-colors focus:border-[#00c2ff]" aria-label="Heure de publication" />;
}
...
export default function App() {
  ...
  const [scheduleDaysOpen, setScheduleDaysOpen] = useState(false);
  useEffect(() => {
    if (!visibilityMenuOpen && !scheduleCalendarOpen && !scheduleDaysOpen && !timezoneMenuOpen) return;
    const closePublicationMenus = event => {
      if (event.target.closest('[data-publication-popover]')) return;
      setVisibilityMenuOpen(false);
      setScheduleCalendarOpen(false);
      setScheduleDaysOpen(false);
      setTimezoneMenuOpen(false);
    };
    document.addEventListener('mousedown', closePublicationMenus);
    return () => document.removeEventListener('mousedown', closePublicationMenus);
  }, [visibilityMenuOpen, scheduleCalendarOpen, scheduleDaysOpen, timezoneMenuOpen]);
              ...
              <div className="max-w-[1240px] mx-auto bg-[var(--bg-surface)] border border-[var(--border-soft)] rounded-3xl p-4 sm:p-8 shadow-2xl space-y-6 sm:space-y-8">
                ...
                {wizardStep === 8 && (() => {
                  ...
                  const timezonePicker = (
                    <div className="relative" data-publication-popover>
                      <button
                        type="button"
                    ...
                    <div className="flex flex-col space-y-6">
                      ...
                      <div className="order-2">
                        ...
                        <div className={`mt-3 space-y-4 ${newChannel.publish_mode === 'auto' || newChannel.publish_mode === 'scheduled' ? '' : 'hidden'}`}>
                          <div className="grid grid-cols-1 gap-2 md:grid-cols-4 [&>div:nth-child(2)]:hidden" data-publication-popover>
                            ...
                            <div><div className="mb-1.5 flex items-center justify-between gap-2"><span className="text-[10px] font-bold text-slate-400">Heure de publication</span><span className="flex gap-1.5"><button type="button" onClick={() => setNewChannel({ ...newChannel, publish_time_mode: 'fixed' })} className={`text-[9px] font-bold ${(newChannel.publish_time_mode || 'range') === 'fixed' ? 'text-[#00c2ff]' : 'text-slate-500'}`}>Fixe</button><span className="text-slate-700">·</span><button type="button" onClick={() => setNewChannel({ ...newChannel, publish_time_mode: 'range' })} className={`text-[9px] font-bold ${(newChannel.publish_time_mode || 'range') === 'range' ? 'text-[#00c2ff]' : 'text-slate-500'}`}>Plage</button></span></div>{(newChannel.publish_time_mode || 'range') === 'fixed' ? <DirectHourInput value={newChannel.publish_schedule_hour ?? 8} onChange={h => setNewChannel({ ...newChannel, publish_schedule_hour: h })} /> : <div className="grid grid-cols-2 gap-1"><DirectHourInput value={newChannel.automation_window_start_hour ?? 7} onChange={h => setNewChannel({ ...newChannel, automation_window_start_hour: h })} /><DirectHourInput value={newChannel.automation_window_end_hour ?? 11} onChange={h => setNewChannel({ ...newChannel, automation_window_end_hour: h })} /></div>}</div>
                          ...
                          <label className="text-[10px] font-bold text-slate-400">Description ajoutée à chaque vidéo
                            <textarea value={newChannel.youtube_default_description || ''} onChange={e => setNewChannel({ ...newChannel, youtube_default_description: e.target.value.slice(0, 4500) })} placeholder="Présentation permanente, liens, avertissements…" className="mt-1.5 min-h-[120px] w-full rounded-xl border border-[var(--border)] bg-[var(--bg-surface-alt)] px-3 py-2 text-xs text-white placeholder:text-slate-600" />
                          </label>
                          <label className="text-[10px] font-bold text-slate-400">Tags permanents
                            <textarea value={(newChannel.youtube_default_tags || []).join(', ')} onChange={e => setNewChannel({ ...newChannel, youtube_default_tags: e.target.value.split(',').map(tag => tag.trim()).filter(Boolean).slice(0, 30) })} placeholder="exemple, chaîne, sujet" className="mt-1.5 min-h-[120px] w-full rounded-xl border border-[var(--border)] bg-[var(--bg-surface-alt)] px-3 py-2 text-xs text-white placeholder:text-slate-600" />
                            <span className="mt-1 block text-[9px] text-slate-500">Les tags IA seront ajoutés automatiquement.</span>
                          </label>
                        </div>
                        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {[
                            ['youtube_made_for_kids', 'Contenu destiné principalement aux enfants', 'Déclaration d’audience envoyée à YouTube.'],
                        ...
                        <div className="w-full"><span className="block text-[10px] font-bold text-slate-400 mb-1.5">Licence</span><div className="flex flex-wrap gap-2">{[['youtube','Licence YouTube standard'],['creativeCommon','Creative Commons']].map(([id,label]) => <button key={id} type="button" onClick={() => setNewChannel({ ...newChannel, youtube_license: id })} className={`min-w-[220px] rounded-xl border px-4 py-2.5 text-center text-[10px] font-bold transition-colors ${(newChannel.youtube_license || 'youtube') === id ? 'border-[#00c2ff] bg-[#00c2ff]/10 text-[#00c2ff]' : 'border-[var(--border)] bg-[var(--bg-surface-alt)] text-slate-300 hover:border-slate-500'}`}>{label}</button>)}</div></div>
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className="bg-[var(--bg-surface)] border border-[var(--border-soft)] rounded-2xl overflow-hidden">
              <div className="p-4 border-b border-[var(--border-soft)] flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-1 bg-[var(--bg-surface-alt)] border border-[var(--border)] rounded-xl p-1 shrink-0">
                  {[{ key: 'list', icon: 'view_list' }, { key: 'grid', icon: 'grid_view' }].map(m => (
                    <button
                      key={m.key}
                      type="button"
                      onClick={() => setAdminVideosViewMode(m.key)}
                      title={m.key === 'list' ? 'Vue en liste' : 'Vue en grille'}
                      className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${adminVideosViewMode === m.key ? 'bg-[#00c2ff] text-slate-950' : 'text-slate-400 hover:text-white'}`}
                    >
                      ...
                      {adminVideos.map(v => (
                        <div key={v.id} className="bg-[var(--bg-surface-alt)] border border-[var(--border)] rounded-2xl overflow-hidden">
                          ...
                          <div className="p-3 space-y-1.5">
                            <button onClick={() => openAdminVideoDetail(v.id)} className="text-xs font-bold text-white hover:text-[#00c2ff] hover:underline text-left line-clamp-2 block w-full">
                              {v.display_title || v.title || '(sans titre)'}
                            </button>
                            {v.youtube_connected ? (
                              <a
                                href={v.youtube_channel_handle ? `https://www.youtube.com/${v.youtube_channel_handle.startsWith('@') ? v.youtube_channel_handle : `@${v.youtube_channel_handle}`}` : `https://www.youtube.com/channel/${v.youtube_channel_id}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-1 text-[10px] text-[#00c2ff] hover:underline"
                              >
                                <YouTubeIcon className="w-3 h-2 shrink-0" />
                                {v.channel_name || '—'}
                              </a>
                            ) : (
                              <p className="text-[10px] text-slate-500 truncate">{v.channel_name || '—'}</p>
                            )}
                        ...
                        <td className="px-4 py-2.5 text-slate-300">
                          {v.youtube_connected ? (
                            <a
                              ...
                              className="inline-flex items-center gap-1.5 text-[#00c2ff] hover:underline"
                            ...
                            <span title="Chaîne YouTube non connectée">{v.channel_name || '—'}</span>
                          )}
                        </td>
            ...
            <div className="flex justify-between items-start gap-3">
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-slate-500 flex items-center gap-1.5 flex-wrap">
                  {adminVideoDetail.youtube_connected ? (
                    <a
                      href={adminVideoDetail.youtube_channel_handle ? `https://www.youtube.com/${adminVideoDetail.youtube_channel_handle.startsWith('@') ? adminVideoDetail.youtube_channel_handle : `@${adminVideoDetail.youtube_channel_handle}`}` : `https://www.youtube.com/channel/${adminVideoDetail.youtube_channel_id}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 text-[#00c2ff] hover:underline"
                    >
                      <YouTubeIcon className="w-3 h-2" />
                      {adminVideoDetail.channel_name || '—'}
                    </a>
                  ) : (
                    <span title="Chaîne YouTube non connectée">{adminVideoDetail.channel_name || '—'}</span>
                  )}
                  · {adminVideoDetail.owner_email || '—'}
                </p>
              </div>
