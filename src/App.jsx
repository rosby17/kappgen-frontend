                      ...
                      <div className="order-2">
                        ...
                        <div className={`mt-3 space-y-4 ${newChannel.publish_mode === 'auto' || newChannel.publish_mode === 'scheduled' ? '' : 'hidden'}`}>
                          <div className="grid grid-cols-1 gap-2 md:grid-cols-4 [&>div:nth-child(2)]:hidden">
                            ...
                            <div><span className="mb-1.5 block text-[10px] font-bold text-slate-400">Heure de publication</span><div className="mb-1 flex gap-1"><button type="button" onClick={() => setNewChannel({ ...newChannel, publish_time_mode: 'fixed' })} className={`text-[9px] font-bold ${(newChannel.publish_time_mode || 'range') === 'fixed' ? 'text-[#00c2ff]' : 'text-slate-500'}`}>Fixe</button><span className="text-slate-700">·</span><button type="button" onClick={() => setNewChannel({ ...newChannel, publish_time_mode: 'range' })} className={`text-[9px] font-bold ${(newChannel.publish_time_mode || 'range') === 'range' ? 'text-[#00c2ff]' : 'text-slate-500'}`}>Plage</button></div>{(newChannel.publish_time_mode || 'range') === 'fixed' ? <HourDropdown fullWidth value={newChannel.publish_schedule_hour ?? 8} onChange={h => setNewChannel({ ...newChannel, publish_schedule_hour: h })} /> : <div className="grid grid-cols-2 gap-1"><HourDropdown fullWidth value={newChannel.automation_window_start_hour ?? 7} onChange={h => setNewChannel({ ...newChannel, automation_window_start_hour: h })} /><HourDropdown fullWidth value={newChannel.automation_window_end_hour ?? 11} onChange={h => setNewChannel({ ...newChannel, automation_window_end_hour: h })} /></div>}</div>
