                      ...
                      <div className="order-2">
                        ...
                        <div className={`mt-3 space-y-3 ${newChannel.publish_mode === 'auto' || newChannel.publish_mode === 'scheduled' ? '' : 'hidden'}`}>
                          <div>
                            ...
                            <div role="radiogroup" aria-label="Visibilité par défaut" className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface-alt)] p-3 space-y-1">{[['private','Privée','lock'],['unlisted','Non répertoriée','link_off'],['members','Réservée aux membres','group'],['public','Publique','public']].map(([id,label,icon]) => { const selected = (newChannel.youtube_privacy_status || 'public') === id; return <button key={id} type="button" role="radio" aria-checked={selected} onClick={() => setNewChannel({ ...newChannel, youtube_privacy_status: id, publish_mode: 'manual' })} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-[11px] transition-colors ${selected ? 'bg-[#00c2ff]/10 text-white' : 'text-slate-300 hover:bg-white/5'}`}><span className={`flex h-5 w-5 items-center justify-center rounded-full border-2 ${selected ? 'border-[#00c2ff]' : 'border-slate-500'}`}>{selected && <span className="h-2.5 w-2.5 rounded-full bg-[#00c2ff]" />}</span><span className="material-symbols-outlined text-[15px] text-slate-400">{icon}</span>{label}</button>; })}</div>
